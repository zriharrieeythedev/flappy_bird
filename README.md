🐦 Flappy Bird: My First Unity Adventure! 🚀

Welcome to Flappy Bird, my very first game built using the Unity Engine and C# scripting! This project is a modern tribute to the classic mobile game that took the world by storm. It features physics-based gameplay, procedurally spawning obstacles, and a responsive score-tracking system.

🎮 Play the Game Now!

You can download and play the latest build of the game directly from Google Drive:

🚀 Download  https://drive.google.com/file/d/1cVksUgo3oKvgNZCPKRj3GRxAlYTmiB61/view?usp=sharing 🚀

Just download the zip file, extract it, and run the executable to start flapping!

✨ Features

Classic Physics Mechanics: Experience the perfect "flap" physics built on Unity's 2D Rigidbody system.

Procedural Pipe Spawning: Pipes spawn endlessly at randomized heights to keep you on your toes.

Real-time Score Tracking: Dodge a pipe, get a point! Watch your score climb at the top of the screen.

Clean Retro UI: Simple game-over screens with a quick restart mechanism.

Fully Animated Sprite: The bird animates beautifully as you flap your wings.

🕹️ How to Play

The rules are simple, but the challenge is real:

Press the Spacebar or Left Mouse Click to flap your wings and defy gravity.

Navigate safely through the gaps in the green pipes 🚧.

Avoid hitting the pipes or falling off the screen, or it's Game Over!

Try to beat your high score! 🏆

🛠️ Built With

Game Engine: Unity

Language: C# (C-Sharp)

Physics: Unity Rigidbody2D & BoxCollider2D

UI System: Unity UI Canvas & TextMeshPro
